...
class BackgroundGenerator(object):
    ...
    def picture(cls, height, width):
        ...
        """
        background_filename = './pictures/Background/'
        pictures = os.listdir(background_filename)# 背景图像的路径

        if len(pictures) > 0:
            picture = Image.open(background_filename + pictures[random.randint(0, len(pictures) - 1)])
